"""Lowe's Pro catalog scraper — Phase C (shipped 2026-05-28).

Source:     Lowe's Companies, Inc., public product detail pages.
            - https://www.lowes.com/c/Pro                    (Pro homepage)
            - https://www.lowes.com/pd/<slug>/<item-number>  (PDP)
License:    Lowe's retailer-proprietary. Public catalog prices only.
            Lowe's Terms of Use prohibit automated scraping at scale.
            This adapter is intended for **low-volume estimating
            spot-checks**, NOT for commercial redistribution of the
            catalog. Each snapshot persists the canonical product URL
            so a downstream consumer can re-verify the value
            interactively before committing it to a bid.
ToS:        https://www.lowes.com/l/terms-and-conditions-of-use.html

What this adapter does
----------------------

Mirrors the contract of ``core/pricing/sources/hd_pro.py``: given a
list of Lowe's Item Numbers, the adapter walks each product page and
emits one ``PricingSnapshot`` per successfully-parsed SKU carrying:

- ``Item Number``           (snapshot ``series_id``)
- ``Title``                 (snapshot ``label``)
- ``Brand``                 (snapshot ``raw["brand"]``)
- ``Price``                 (snapshot ``value``, USD)
- ``Unit-of-measure``       (snapshot ``unit`` — verbatim where present,
                             e.g. ``"each"``, ``"box of 100"``,
                             ``"case of 50"``, ``"1 lb"``; falls back
                             to ``"each"``)
- ``Model Number``          (snapshot ``raw["mpn"]``)
...
**Lowe's serves its product pages behind an Akamai-fronted JavaScript
shell with bot protection** — the same posture as Home Depot. A polite
``httpx`` GET (no JavaScript engine, no cookie / fingerprint mimicry)
will most often land on an "Access Denied" / "Pardon Our Interruption"
interstitial, an HTTP 403, or a 200 response carrying only the JS
shell.
...
alongside the matching ``hd_pro`` notes. The adapter degrades
gracefully with the same posture as ``hd_pro``:
...
3. When the response *is* a CAPTCHA / anti-bot page (200-OK HTML
   carrying one of the known interstitial markers), the adapter logs
   a warning and skips that SKU rather than emitting a misleading
   snapshot.

Recommended remediation paths, in preference order (none implemented
in this slice):

(a) **Use Lowe's official supplier / B2B Pro account API** when one is
    available. Requires a LowesForPros.com Pro account + commercial
    contract.
(b) **Use a 3rd-party catalog aggregator** (Datalink, Build.com,
    similar) with explicit license to redistribute pricing for
    estimating use.
...
- 24h disk cache via ``_cci_common.http_get_text``; cache lives under
  ``config/pricing_snapshots/_http_cache/lowes_pro/`` (gitignored).
...
- No bulk-download attempt: the SKU list is bounded at construction
  ...
  ``scripts/refresh_pricing.py::LOWES_PRO_STARTER_SKUS``).
...
- ``source``:        "lowes_pro"
- ``series_id``:     "<Item Number>"      # parsed ``meta itemprop="sku"``,
...
- ``period``:        "YYYY-MM-DD"         # scrape date — prices fluctuate
                                            hourly during promo seasons
- ``license``:       per ``license_str`` below; cite at every use
- ``source_url``:    https://www.lowes.com/pd/<sku>
...
LOWES_PRO_HOMEPAGE = "https://www.lowes.com/c/Pro"
...
# These are illustrative Lowe's Item Numbers — the live Lowe's catalog
# is the source of truth, and these IDs may be retired or repriced at
# any time. The refresh runner overrides this list with its own
# ``LOWES_PRO_STARTER_SKUS`` constant so the policy lives at the call
# site (see ``scripts/refresh_pricing.py``).
DEFAULT_SKUS: list[str] = [
    "1099113",   # 3/4 in. PVC pipe (illustrative)
    "12533",     # 2x4x8 SPF framing lumber
    "73268",     # drywall screws, 1 lb box
    "12174",     # 1/2 in. sheathing plywood
    "1003356",   # interior latex paint, 1 gal
    "47546",     # passage door knob
    "303116",    # exterior door hinge
    "23005",     # 5 gal joint compound
    "10243",     # 4x8 gypsum board
    "12553",     # 30 lb roofing felt
]


def product_url(sku: str, slug: Optional[str] = None) -> str:
    """Canonical Lowe's product URL for ``sku``.

    Real-world Lowe's product URLs include a marketing slug:
    ``/pd/<slug>/<item-number>``. For programmatic fetches the slug is
    optional; Lowe's redirects bare-Item-Number URLs to the canonical
    slug page. We use the bare-SKU form so we don't need to pre-know
    the slug per SKU.
    """
    ...
    if slug:
        return f"{LOWES_BASE_URL}/pd/{slug}/{sku_clean}"
    return f"{LOWES_BASE_URL}/pd/{sku_clean}"
...
class LowesProSource(PricingSource):
    """Lowe's Pro catalog scraper.
    ...
    license_str = (
        "Lowe's retailer-proprietary; public catalog prices only — "
        "low-volume estimating use, not commercial redistribution"
    )
    homepage_url = LOWES_PRO_HOMEPAGE
    ...
    def fetch(
        ...
        - If ``series_ids`` is non-empty each id is treated as an
          explicit Item Number and fetched.
    ...
    def fetch_one(
        ...
        sku_clean = (sku or "").strip()
        if not sku_clean:
            raise PricingSourceUnavailable("Lowe's Pro: empty sku")
        ...
        try:
            ...
            raise PricingSourceUnavailable(
                f"Lowe's Pro: HTTP error fetching {url}: {exc}"
            ) from exc
        ...
        if snap is None:
            raise PricingSourceUnavailable(
                f"Lowe's Pro: could not parse product page at {url} "
    ...
    def _fetch_one_safe(
        ...
        try:
            ...
            LOG.warning("Lowe's Pro: skipping sku %r — %s", sku, exc)
            return None
        except Exception as exc:  # noqa: BLE001
            LOG.warning(
                "Lowe's Pro: unexpected error fetching sku %r — %s", sku, exc,
            )
            return None
