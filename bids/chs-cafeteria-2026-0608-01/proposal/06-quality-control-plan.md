...
> **Approach:** Three-phase inspection (prep / in-progress / completion) per the US Army Corps of Engineers "3-phase QC" pattern that {{AGENCY_SHORT}} System UGSC has historically required. Article reference: `[TBD per CSP package — typically {{AGENCY_SHORT}} SGC Article 7 "Quality of Work" / "Contractor Quality Control" or equivalent. The 2010 TFC UGSC base (in the Angelo State packet Attachment C) carries this in Article 7 and Article 8; {{AGENCY_SHORT}} SGC overlays may shift the article number.]`
...
|---|---|---|
| QC Manager (overall) | `[USER TO FILL — typically the Project Manager or a dedicated QC lead]` | Authors the project QC plan; reviews submittals against specs; chairs preparatory + initial + follow-up inspection meetings; signs off on each phase. |
| QC field representative | `[USER TO FILL — typically the Project Superintendent]` | Performs daily QC walks; documents each three-phase inspection with photos; tracks corrective actions to closure. |
| Independent testing agency (if required) | `[USER TO FILL — typically the owner's third-party testing agency for concrete, soils, fireproofing; not common for a single-room interior reno but verify against CSP package]` | Independent verification testing per the specs. |
...
Held **before** the trade starts on-site. Attendees: QC Manager, QC field rep, the subcontractor's foreman, the A/E ({{AE_FIRM_NAME}}) if requested, the {{ISSUING_OFFICE}} PM ({{PM_NAME}}) on a notice basis.
...
Held **throughout** the trade's on-site work (regular cadence) and at **completion** (before A/E punch list). Same attendees, but the {{ISSUING_OFFICE}} PM is more likely to attend the completion inspection.
...
1. **Internal pre-punch walk** by QC field rep + Project Manager (typically Day 88 of schedule). Internal punch list captured in a project-management tool (Procore, BlueBeam Studio, or PDF markup). Subcontractors notified of their items same-day.
2. **Sub-by-sub clearance** before the A/E walk — every sub closes their pre-punch items first.
3. **A/E + owner punch walk** (typically Day 92). Attendees: {{AE_FIRM_NAME}}, {{ISSUING_OFFICE}} PM ({{PM_NAME}}), {{AGENCY_SHORT}} Facilities representative, {{AGENCY_SHORT}} EHS representative, our PM + QC field rep. List captured at the walk; sub notifications sent within 24 hours.
...
- **Submittal log** maintained in project-management tool from Day 1; targeted 10-business-day A/E turnaround (assumed; adjust per CSP package `[PENDING e-BUILDER ACCESS]`). Critical-path submittals (lab casework, fume hood if scope, lab fixtures) submitted by Day 5–7 post-NTP.
...
- **Distribution:** all submittal + RFI logs distributed to {{ISSUING_OFFICE}} PM ({{PM_NAME}}) weekly with the project status report.

---

## F. Owner-facing QC reporting

- **Weekly project status report** distributed to {{PM_NAME}}, Fred {{AE_FIRM_NAME}}, and the firm's Project Executive every Monday morning. Includes: % complete by phase, schedule status (on / ahead / behind), open RFI count + aging, open submittal count + aging, open punch items, safety incidents, EHS coordination notes, photo highlights.
- **Monthly pay application** with HUB Progress Assessment Report (PAR) per {{AGENCY_SHORT}} HUB Operations standard, filed with {{HUB_POC_NAME}}. Pay app submission discipline: complete G702/G703 (or {{AGENCY_SHORT}}-equivalent form `[PENDING e-BUILDER ACCESS]`) with sworn statement, lien waivers from all paid subs, photos of work-in-place, and HSP progress narrative.
- **Closeout documentation** per the {{AGENCY_SHORT}} SGC closeout requirements `[PENDING e-BUILDER ACCESS — typically O&M manuals, attic stock, training records, warranties, as-built drawings, EHS final clearance letter]`.

---

## G. Continuous-improvement loop

After substantial completion, the QC Manager holds a 1-hour internal lessons-learned session and produces a 1-page memo of "what worked / what didn't / what we'll do differently next time." The memo is filed in the firm's project archive and informs the QC plan for the next {{AGENCY_SHORT}} System project.
...
- {{AGENCY_SHORT}} System UGSC quality requirements `[PENDING e-BUILDER ACCESS: confirm exact article reference]`
- AIA A201 General Conditions Article 3.5 (Warranty) and 12 (Uncovering and Correction of Work) baseline expectations

If the CSP package requires a specific QC plan format (some {{AGENCY_SHORT}} CSPs prescribe a 1-page form; some require a multi-page narrative), this plan will be re-formatted on G2 close.
