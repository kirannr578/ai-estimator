# CHS Cafeteria Serving Line Renovation — HUB Subcontracting Plan (HSP)
...
| Applicable category | `N/A (ISD CSP — Tex. Educ. Code §44.031 governs; no HSP required)` (heavy / building / special-trade / professional / other) |
| Statewide goal | `N/A%` |
| Project-specific override | `N/A%` `[USER TO FILL — confirm via RFI if not explicit]` |
| **HUB commitment in BPC's HSP** | **`N/A%`** (≥ applicable goal) |
...
| **Total HSP commitment** | | | | **$`{{HUB_TOTAL_$}}` (`N/A%` of subcontracted Work)** |
...
2. Filter by HUB-certified + NIGP/NAICS for trade + `Johnson` + adjacent counties + statewide for specialty
...
| Contacted HUB Discretionary Contracting Forum / `Cleburne ISD` HUB Operations | ☐ | `{{CONTACT_RECORD}}` |
...
- Notify the `Cleburne ISD` HUB Office in writing of any sub substitution
- If actual HUB participation falls short of the HSP commitment, file a revised HSP (with `Cleburne ISD` concurrence) or document the reason and continue GFE

## 8. HSP submission

| Field | Value |
|---|---|
| HSP submission method | `N/A` (with proposal / separate deadline) |
| HSP submission deadline | `N/A (no HSP required)` @ `N/A` `N/A` |
| HSP form template used | `N/A (no HSP required for ISD CSP)` (per CSP — DO NOT use a prior bid's form) |
| HSP signer | `N/A`, `N/A` |
| HSP signing date | `N/A` |
