...
| Metric | Value |
|---|---|
| Estimated magnitude (often not published in NOP) | $`{{MAGNITUDE_LOW}}` – $`{{MAGNITUDE_HIGH}}` `[USER TO FILL — estimate from scope]` |
...
- **TX prevailing-wage uplift is real.** Verify firm + sub labor rates against `{{TX_WAGE_FILE}}` for `{{COUNTY}}`.
