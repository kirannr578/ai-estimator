# {{PROJECT_NAME}} — Texas Prevailing-Wage Checklist
...
> **County:** `{{COUNTY}}` County, Texas

## 1. Wage-table source

| Item | Value |
|---|---|
| Wage table source | `{{WAGE_TABLE_SOURCE}}` (per CSP Attachment / TFC / county / TX Workforce Commission) |
| Wage table filename | `{{TX_WAGE_FILE}}` |
| Wage table effective date | `{{WAGE_TABLE_DATE}}` |
| Wage table archived in workspace at | `bids/{{SLUG}}/wage-table/` |

## 2. Trade rates applied (per CSI / per CLIN)

| Trade | CSI division | Wage classification | Hourly rate per `{{TX_WAGE_FILE}}` | Fringe per WD | Total rate | BPC firm rate | Uplift if firm < prevailing |
|---|---|---|---|---|---|---|---|
...
- [ ] If `{{AGENCY_SHORT}}` has additional prevailing-wage clauses in SGC, comply with both
