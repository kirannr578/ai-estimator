...
`SSC 2025-06813 — Harrington Education Center Lab 303 Renovation — proposer eligibility confirmation + CSP package access + site walk request`
...
We have the public Notice of Project in hand and have begun preparing. Before
we go further, I have three time-sensitive asks and one logistics request.

1. PROPOSER ELIGIBILITY POST-MISSED PRE-PROPOSAL MEETING

   The Notice lists a pre-proposal meeting at SSC Facilities Services Conf
   Rm S118 on 2026-05-14 at 2:00 PM CT. We were not able to attend that
   meeting. The Notice does not state whether attendance was mandatory.

   Could you please confirm in writing whether the 5/14 pre-proposal meeting
   was mandatory for proposers, and — if mandatory — whether a firm that did
   not attend may still submit a responsive proposal under any late-attendance
   or written-summary-acknowledgment accommodation?

   This is the single most important question for us before we sink further
   resources into preparation. If we are categorically ineligible, we would
   rather know now and notify you of a no-bid as a courtesy.

2. PRE-PROPOSAL MEETING RECORD

   Independent of the eligibility question above: could you please share the
   pre-proposal meeting sign-in sheet, recording (if any), written Q&A, and
...
4. SITE WALK REQUEST
   ...
   We would very much appreciate the opportunity to walk Lab 303 prior to
   final pricing, ideally during the week of 5/25 (or as soon thereafter as
   site access can be arranged). Existing-conditions risk is substantial on a
   science classroom-lab renovation, and a 30-minute walk-through would let
   us validate room dimensions, lab utility configuration, and adjacent-lab
   coordination requirements.

   Could you advise on the process for scheduling a site walk through SSC, and
   whether Patterson Architects (Fred Patterson) typically attends?

For your reference, our team for this bid:

   Project Executive: [USER TO FILL: name + title + phone + email]
...
| Joelle says | Action |
|---|---|
| "5/14 was non-mandatory; you can bid" | Proceed. G1 closes green. Continue with the rest of the plan in `../timeline.md`. |
| "5/14 was mandatory; here is the late-arriver accommodation: [...]" | Verify the accommodation is legitimate and documentable; if yes, proceed; if no, walk. |
| "5/14 was mandatory; no exceptions" | **WALK AWAY.** Send Joelle a no-bid courtesy reply. Archive folder. |
| "I'm checking; will let you know by [date]" | Hold non-critical work; continue HUB outreach and CSP-portal-pull in parallel; expect a 24-hour response. |
| No reply for 48 hours | Phone, then escalate to Patty Winkler, then TAMU System OFPC. |
