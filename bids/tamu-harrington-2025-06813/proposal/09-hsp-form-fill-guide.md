...
- Texas state agencies (including TAMU) require an HSP when subcontracting opportunities exist
- The applicable HUB participation goal varies by procurement class — for renovation work, the **statewide 21.1% goal for "special trade construction"** applies unless TAMU has set a project-specific higher goal (`[USER TO FILL — confirm with Patty Winkler]`)
...
`[PENDING e-BUILDER ACCESS or Patty Winkler confirmation: download the actual TAMU HSP form and re-author this guide section-by-section against the actual fields.]`
...
| HUB goal applicable | `21.1%` (statewide special-trade construction goal per 34 TAC §20.284, current as of 2024 amendment) — `[USER TO FILL: confirm vs project-specific goal from Patty Winkler]` |
...
1. **CMBL search records** — screenshot of search filtered by HUB-certified + NIGP/NAICS for that trade + Brazos County area; dated.
...
| PAR recipient | `Patty Winkler, TAMU HUB Operations, p-winkler@tamu.edu` (per Notice) |
| PAR cadence | Monthly with each pay application (per Notice) |
| HSP change-management procedure | Per 34 TAC §20.285; prime must request TAMU HUB Operations approval for any HSP modification post-award |

### C-7. Affirmation + signature

| Field | Value to enter |
|---|---|
| Affirmation language | (standard TX HSP affirmation; verify exact text against actual form `[PENDING — Patty Winkler]`) |
...
- [ ] HSP separately delivered to Patty Winkler if TAMU requires both Proposal Form delivery and direct HSP delivery `[PENDING — confirm with Patty]`
...
6. **Form on wrong template.** TAMU uses its own header; do not submit on a non-TAMU template once TAMU's is in hand.

---

## F. Workflow when TAMU HSP form lands

1. Download the actual TAMU HSP form (from Patty Winkler or the e-Builder portal).
2. Compare against the State of Texas standard HSP form and the Angelo State Attachment D HSP form. Note any TAMU-specific additions (commitment-letter format, narrative-length limits, attestation language).
3. Re-author this guide section-by-section against the actual TAMU form.
