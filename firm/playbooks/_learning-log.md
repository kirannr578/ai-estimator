...
## 2026-05-30 (continued) — `private-gc-sub-bid.md` playbook authored (Worker C)

**Trigger:** Three-time-reinforced gap in the running log (today's DFWMSDC entry, today's DFWMSDC followup recommendation, today's continued McLennan + Camp Maxey reinforcement). Playbook authored in response.

- **Playbook created:** [`firm/playbooks/private-gc-sub-bid.md`](private-gc-sub-bid.md). Covers BPC's posture as a sub-quoter to a private GC (vs. BPC as prime to a public owner): GC-portal mechanics (BuildingConnected, BuildBid, Procore Bid Board, PlanGrid, iSqFt, DemandStar, HCS Box, GC's own portals); the 5-step pursue checklist; pay-when-paid vs. pay-if-paid clause distinction under Tex. Bus. & Com. Code Ch. 56 + Trust Fund Act (Tex. Prop. Code Ch. 162) + lien rights (Tex. Prop. Code Ch. 53); sub-tier insurance posture (CG 20 10 + CG 20 37 + P&NC + waiver of subrogation; restaurant-chain $5M umbrella); top-5 risk register; pricing posture (open-shop OEWS for pure-private vs. DBRA / TWC carry for public-pass-through).
- **Index updated:** [`firm/playbooks/README.md`](README.md). Added a new row to the playbook index citing the live BCI Belisle / Longhorn Little Elm watchlist + the three other private-GC sub-bid workspaces from this week's ingest as exemplars. Replaced the "Private commercial / institutional negotiated — *not playbook-grade; ad-hoc*" row in the procurement-type matrix with two rows: (a) the new "Private GC sub-bid" playbook-grade row pointing to `private-gc-sub-bid.md`, and (b) a renamed "Private commercial / institutional negotiated — BPC as prime" ad-hoc row for the Hindu Temple / Holiday Inn / Lavon RV Park lineage where BPC is the prime, not the sub.
- **TX private-construction-law nuance parked for legal review:** the playbook leaves explicit `[USER TO FILL — verify with counsel]` markers at two spots — (1) the Tex. Bus. & Com. Code Ch. 56 prompt-payment timing default (chapter restructured in 2019; pay-when-paid case law under the restructured chapter is still developing); (2) Tex. Prop. Code Ch. 53 mechanic's-lien filing windows (chapter restructured in 2021; windows changed). Both are non-blocking for using the playbook on pure-private restaurant fit-outs (the GC's subcontract governs day-to-day) but matter when the relationship goes adversarial. Surface to counsel before the first sub-bid where BPC is asked to sign a pay-if-paid clause with unambiguous condition-precedent language.
- **Commit + push status:** `[USER TO FILL — commit SHA inserted by Worker C after `git push origin main` completes]`. Sibling workers A/B/D pushed concurrently to `firm/scope-templates/` and `core/` + `tests/` paths; this worker did not touch those paths.
- **Followups:**
