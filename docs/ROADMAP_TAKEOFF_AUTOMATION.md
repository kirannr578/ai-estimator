...
**Suggested next slices:**
- **Phase T6.2 — Streamlit `MANUAL_OVERRIDE` UI.** Wire `apply_manual_override` into the Streamlit "Recalculate totals" round-trip so an operator can hand-price a HAND_TAKEOFF row directly in the UI. Should use the operator-note field for vendor / source attribution.
